...
export function OnboardingGateway() {
  ...
  return (
    <div
      role="dialog"
      aria-modal="true"
      className={`fixed inset-0 z-[100] flex items-center justify-center p-3 sm:p-6 transition-all duration-400 ease-in-out bg-background/20 ${
        isExiting ? 'opacity-0 scale-95 blur-md' : 'opacity-100 scale-100 blur-0 animate-fade-in'
      }`}
    >
      <div
        className={`w-full max-w-5xl rounded-3xl p-5 sm:p-7 md:p-8 glass-card border border-outline-variant/60 flex flex-col items-center gap-5 sm:gap-7 transition-all duration-400 bg-surface-container-lowest/80 ${
          isExiting ? 'scale-95' : 'animate-scale-up'
        }`}
        style={{
