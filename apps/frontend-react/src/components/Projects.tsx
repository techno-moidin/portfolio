...
export function Projects() {
  ...
  // ── 1. Keyword Matcher Event Listener ──────────────────────────────────────
  useEffect(() => {
  ...
  // Trigger initial calculation on load for CEO
  useEffect(() => {
    if (role === 'CEO') {
      calculateScope();
    }
  }, [role, complexity, timeline, scale]);

  // ── 3. CTO WebSockets Telemetry integration ────────────────────────────────
  useEffect(() => {
    if (role !== 'CTO') {
      if (socket) {
        socket.disconnect();
    ...
    logEvent('WS', 'Connecting to Telemetry WebSocket Gateway...');
    ...
    newSocket.on('connect', () => {
      setWsConnected(true);
      logEvent('WS', 'Connected to ws://localhost:3000 telemetry gateway.');
    });
    ...
    newSocket.on('disconnect', () => {
      setWsConnected(false);
      logEvent('WS', 'Disconnected from ws://localhost:3000.');
    });
  ...
  // Handle local simulation when WS is offline
  useEffect(() => {
    if (role !== 'CTO' || wsConnected) return;
  ...
  }, [role, wsConnected, traffic, redisEnabled, workerNodes]);

  // WS trigger adjusters
  ...
  const provisionNodeLocal = () => {
    let nextNode = workerNodes < 8 ? workerNodes + 1 : 1;
  ...
  return (
    <section className="py-section-gap bg-surface-container-lowest/30 relative reveal" id="projects">
      ...
      {role === 'CTO' && (
        <div className="max-w-container-max mx-auto px-gutter mb-4">
          <div className="flex items-center gap-2 font-mono text-[10px] text-sky-400 bg-sky-950/40 border border-sky-800/40 px-3 py-1 rounded-md max-w-fit">
            ...
            <span>CONNECTED TO TELEMETRY MATRIX: ws://localhost:3000 | POST /api/portfolio/calculate-scope</span>
          </div>
        </div>
      )}
      ...
      <div className="max-w-container-max mx-auto px-gutter">
        ...
        {/* 🚀 CEO PERSPECTIVE: Project Scope & ROI Calculator */}
        {role === 'CEO' && (
        ...
        {/* 💻 CTO PERSPECTIVE: System Architecture Stress-Tester */}
        {role === 'CTO' && (
          <div className="glass-card p-6 border border-sky-500/30 rounded-xl mb-12 shadow-xl bg-surface-container-lowest/50">
